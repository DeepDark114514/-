...
# 2026.5.17
# B方案训练入口：DegFiLM-ResUNet（盲QP视频压缩伪影去除）
# 支持对照组切换:
    # --use_film 1  → B方案 (DegFiLM-ResUNet，带 FiLM 注入)
    # --use_film 0  → 对照组 (PureResUNet + 多QP数据，隔离"数据增益"与"模块增益")
# 与 train.py 的主要区别:
    # 1. 使用 MultiQPDataset / build_multi_qp_dataloader（多QP分层采样）
    # 2. 训练返回的 batch 含 qp_tensor，用于记录各QP loss 分布
    # 3. 支持验证时在多个 unseen QP 上评估（验证盲QP自适应能力）
    # 3. 新增 --use_film 命令行开关
# 用法示例:
    # # B方案训练（多QP + FiLM）
    # python train_b.py --use_film 1 --base_ch 32 --epochs 100
    # # 对照组（多QP + 纯残差UNet）
    # python train_b.py --use_film 0 --base_ch 32 --epochs 100
    # # 验证 unseen QP（25/30/35/40）
    # python train_b.py --use_film 1 --eval_only --checkpoint logs/B_xxx/best_model.pth
...
from utils import EarlyStopping, calc_psnr, set_high_priority


def parse_args():
    # 命令行参数解析
    parser = argparse.ArgumentParser(description='B方案训练框架 (DegFiLM-ResUNet)')
    parser.add_argument('--use_film', type=int, default=1, choices=[0, 1],
                        help="1=B方案(DegFiLM-ResUNet), 0=对照组(PureResUNet+多QP)")
    ...
    parser.add_argument('--unseen_qp', type=int, nargs='+', default=None,
                        help="验证时额外评估的 unseen QP 列表，如 25 30 35 40")
    return parser.parse_args()


def override_config(cfg, args):
    # 用命令行参数覆盖 config
    cfg['use_film'] = bool(args.use_film)
...
def train_one_epoch(model, dataloader, loss_fn, optimizer, scaler, device, cfg, epoch, logger):
    ...
    for step, batch in enumerate(pbar):
        ...
        if step % 50 == 0:
            logger.info(
                f"Epoch {epoch}/{cfg['epochs']}, Batch {step+1}/{len(dataloader)}, "
                f"Loss: {loss.item() * accum_steps:.4f}, L1: {l1_val:.4f}, "
...
def setup_logger(exp_dir):
    ...
    log_file = os.path.join(exp_dir, f'train_b_{timestamp}.log')

    logger = logging.getLogger('train_b')
...
def main():
    ...
    if args.resume:
        ...
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        flag = 'B' if cfg['use_film'] else 'B_ctrl'
        exp_name = f"{flag}_{timestamp}"
    ...
    logger.info(f"B方案训练 | use_film={cfg['use_film']} | "
                f"模型: {'DegFiLM-ResUNet' if cfg['use_film'] else 'PureResUNet(对照)'}")
    ...
    if cfg['use_film']:
        ...
        logger.info(f"  - 相对 A方案基线 ({base_params / 1e6:.2f}M) 增加: {pct:.2f}%")
    ...
    # 训练：多QP分层采样
    train_loader = build_multi_qp_dataloader(cfg, 'train', qp_list=train_qp)
    # 验证：固定QP（默认32）
    val_loader = build_multi_qp_dataloader(cfg, 'val', qp_list=val_qp)
    ...
    logger.info(f"早停 patience={cfg['early_stop_patience']}, "
                f"min_delta={cfg['early_stop_min_delta']}, mode={cfg['early_stop_mode']}")
    ...
    if args.eval_only:
        ...
        # 在 unseen QP 上评估（如果配置）
        unseen_qps = cfg.get('unseen_qp_list', [])
        for uqp in unseen_qps:
            uqp_loader = build_multi_qp_dataloader(cfg, 'val', qp_list=uqp)
    ...
    if args.resume and os.path.exists(csv_path):
        ...
        with open(csv_path, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow([
                'epoch', 'train_loss', 'train_l1', 'train_ssim', 'train_psnr',
    ...
    for epoch in range(start_epoch, cfg['epochs'] + 1):
        ...
        logger.info(
            f"Epoch {epoch}/{cfg['epochs']} training summary - "
            f"Loss: {train_loss:.6f}, L1: {train_l1:.6f}, SSIM: {train_ssim:.4f}, "
            f"PSNR: {train_psnr:.2f}, Grad Norm: {grad_norm:.4f}"
        )
        ...
        if epoch % cfg['val_interval'] == 0 or epoch == cfg['epochs']:
            ...
            logger.info(
                ...
                f"Val PSNR: {val_psnr:.4f} dB, Val SSIM: {val_ssim:.4f}, "
            ...
            if cfg['early_stop']:
                ...
                logger.info(
                    f"早停状态 - 最佳PSNR: {early_stopper.best_score:.4f}, "
                    f"停滞计数: {early_stopper.counter}/{cfg['early_stop_patience']}"
                )

                if should_stop:
                    logger.info(
                        f"Early stopping triggered at epoch {epoch} "
                        f"(best val PSNR: {early_stopper.best_score:.4f} dB)"
                    )
                    break
            else:
                if val_psnr > best_psnr:
